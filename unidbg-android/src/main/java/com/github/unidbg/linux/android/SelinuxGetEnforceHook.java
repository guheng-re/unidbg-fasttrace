...
/**
 * Native {@code libselinux.so} {@code security_getenforce(void)} only.
 * When {@code android.securitySignals} is configured, returns {@code 1} if
 * {@code selinuxEnforced} is true else {@code 0} and emits one {@code linux_security}
 * sidecar. When the node is absent, does not intercept (no host fallback).
 * Does not implement is_selinux_enabled, setenforce, getfilecon, xattr, getcon, or other APIs.
 */
public class SelinuxGetEnforceHook implements HookListener {

    private static final Logger log = LoggerFactory.getLogger(SelinuxGetEnforceHook.class);

    public static final String LIBRARY = "libselinux.so";
    public static final String SYMBOL = "security_getenforce";

    private final Emulator<?> emulator;

    public SelinuxGetEnforceHook(Emulator<?> emulator) {
        this.emulator = emulator;
    }

    /**
     * Config-backed {@code security_getenforce}: returns {@code 0} or {@code 1} when
     * {@code android.securitySignals} is present; {@code null} when absent (caller keeps
     * prior path; no host fallback; no sidecar).
     */
    public static Integer trySecurityGetenforce(Emulator<?> emulator) {
        if (emulator == null) {
            return null;
        }
        ...
        int result = signals.isSelinuxEnforced() ? 1 : 0;
        TraceEnvironmentEventSink.emit(emulator, "linux_security",
                "security_getenforce",
                "result=" + result,
                "json-config", "读取配置的 SELinux 强制模式（native security_getenforce）");
        ...
        if (emulator.is64Bit()) {
            return svcMemory.registerSvc(new Arm64Hook() {
                ...
                protected HookStatus hook(Emulator<?> emulator) {
                    return handleGetEnforce(emulator, old);
                }
            }).peer;
        }
    ...
    private static HookStatus handleGetEnforce(Emulator<?> emulator, long old) {
        Integer result = trySecurityGetenforce(emulator);
        if (result != null) {
            if (log.isDebugEnabled()) {
                log.debug("security_getenforce result={}", result);
            }
