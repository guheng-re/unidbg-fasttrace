...
public class ARM64SyscallHandler extends AndroidSyscallHandler {
        ...
        try {
            ...
            switch (NR) {
                ...
                case 62:
                    ...
                    return;
                case 172: // getpid
                    backend.reg_write(Arm64Const.UC_ARM64_REG_X0, emitProcessIdentity(emulator, "getpid", emulator.getPid()));
                    return;
                case 178: // gettid
                    Task task = emulator.get(Task.TASK_KEY);
                    backend.reg_write(Arm64Const.UC_ARM64_REG_X0, emitProcessIdentity(emulator, "gettid", getConfiguredTid(emulator, task == null ? 0 : task.getId())));
                    return;
                case 129:
                ...
                case 44: {
                    ...
                    Pointer buf = context.getPointerArg(1);
                    backend.reg_write(Arm64Const.UC_ARM64_REG_X0, fstatfs64(emulator, fd, buf));
                    return;
                }
                case 46: {
                ...
                case 73:
                    ...
                    return;
                case 90: { // capget (linux aarch64)
                    Integer capgetRet = tryCapget(emulator);
                    if (capgetRet != null) {
                        backend.reg_write(Arm64Const.UC_ARM64_REG_X0, capgetRet.intValue());
                        return;
                    }
                    break;
                }
                case 155: { // getpgid(pid)
                    Integer pgidRet = tryGetpgid(emulator);
                    if (pgidRet != null) {
                        backend.reg_write(Arm64Const.UC_ARM64_REG_X0, pgidRet.intValue());
                        return;
                    }
                    break;
                }
                case 156: { // getsid(pid)
                    ...
                    if (sidRet != null) {
                        backend.reg_write(Arm64Const.UC_ARM64_REG_X0, sidRet.intValue());
                        return;
                    }
                    break;
                }
                case 158: { // getgroups
                    ...
                    if (groupsRet != null) {
                        backend.reg_write(Arm64Const.UC_ARM64_REG_X0, groupsRet.intValue());
                        return;
                    }
                    syscall = "getgroups";
                    break;
                ...
                case 173:
                    ...
                    return;
                case 174: // getuid
                    backend.reg_write(Arm64Const.UC_ARM64_REG_X0, emitProcessIdentity(emulator, "getuid", getConfiguredUid(emulator, 0)));
                    return;
                case 175: // geteuid
                    backend.reg_write(Arm64Const.UC_ARM64_REG_X0, emitProcessIdentity(emulator, "geteuid", getConfiguredEuid(emulator, 0)));
                    return;
                case 176: // getgid
                    backend.reg_write(Arm64Const.UC_ARM64_REG_X0, emitProcessIdentity(emulator, "getgid", getConfiguredGid(emulator, 0)));
                    return;
                case 177: // getegid
                    backend.reg_write(Arm64Const.UC_ARM64_REG_X0, emitProcessIdentity(emulator, "getegid", getConfiguredEgid(emulator, 0)));
                    return;
                case 200:
                ...
                case 163:
                    ...
                    return;
                case 179:
                    backend.reg_write(Arm64Const.UC_ARM64_REG_X0, sysinfo(emulator));
                    return;
                case 198:
    ...
    /** Linux {@code RLIMIT_NOFILE}: max number of open files. */
    ...
    private long getrlimit64(Emulator<AndroidFileIO> emulator) {
        ...
        if (resource == RLIMIT_NOFILE) {
            ...
            if (config != null && config.isLinuxRlimitsNofileConfigured()) {
                TraceEnvironmentConfig.LinuxRlimitsConfig rlimits = config.getLinuxRlimitsConfig();
                long soft = rlimits.getNofileSoft();
                long hard = rlimits.getNofileHard();
                RLimit64 rlimit64 = new RLimit64(ptr);
                rlimit64.rlim_cur = soft;
                rlimit64.rlim_max = hard;
                rlimit64.pack();
                TraceEnvironmentEventSink.emit(emulator, "linux_proc", "getrlimit64",
                        "resource=" + resource + ",soft=" + soft + ",hard=" + hard,
                        "json-config",
                        "读取配置的 getrlimit64 RLIMIT_NOFILE");
                return 0;
            }
        }
    ...
    protected int uname(Emulator<?> emulator) {
        ...
        sysName.setString(0, sysnameValue); /* Operating system name (e.g., "Linux") */
        ...
        String machineValue = config == null ? "aarch64" : config.getUnameMachine(true, "aarch64");
        Pointer machine = version.share(SYS_NMLN);
        machine.setString(0, machineValue); /* Hardware identifier */
    ...
    private int prctl(Emulator<?> emulator) {
        ...
        switch (option) {
            case PR_GET_DUMPABLE: {
                ...
                // Historical path when dumpable is not configured: unsupported
            ...
            case PR_SET_NAME:
                ...
                return 0;
            case PR_GET_NAME: {
                // Match ARM32: process.threadName with current Java thread-name fallback;
    ...
    protected int fstat(Emulator<?> emulator, int fd, Pointer stat) {
        ...
        Stat64 statStructure = new Stat64(stat);
